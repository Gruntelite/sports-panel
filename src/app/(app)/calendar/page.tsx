              ...
              <div key={day} className="p-2 bg-card min-h-[120px] flex flex-col gap-1">
                ...
                {dayEvents.map(event => (
                  <div key={event.id} className="text-xs p-1 rounded-md bg-muted">
                    ...
                    <Badge variant={event.type === 'Match' ? 'destructive' : 'secondary'} className="w-full justify-center mt-1">{event.type}</Badge>
                  </div>
                ))}
              </div>
            )
...
export default function CalendarPage() {
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold font-headline tracking-tight">Event Calendar</h1>
        <p className="text-muted-foreground">
          View and manage all training sessions and matches.
        </p>
      </div>
      <CalendarView />
    </div>
