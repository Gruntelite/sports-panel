...
export default function FeesPage() {
  ...
  const [platformCommissionCents, setplatformCommissionCents] = useState<number>(24);
  ...
  const MONTHS = t("months", { returnObjects: true }) as {
    label: string;
    value: number;
  }[];
  ...
  const filteredPlayers = useMemo(() => {
    return players.filter((player) => {
      const nameMatch = `${player.name} ${player.lastName}`
        .toLowerCase()
    ...
    const processPage = async (user: any) => {
      try {
        ...
        if (settingsSnap.exists()) {
          ...
          setplatformCommissionCents(240);
          setFeeChargeMonths(settingsData.feeChargeMonths || []);
          setOnboardingComplete(
            settingsData.stripeConnectOnboardingComplete || false
          );
        }
        ...
        if (
          ...
          searchParams.get("clubId") === currentClubId
        ) {
          ...
          toast({
            ...
            description: "Tu cuenta de Stripe se ha conectado correctamente.",
          });
        ...
        const teamsQuery = query(
          collection(db, "clubs", currentClubId, "teams"),
          orderBy("order")
        );
        ...
        const playersQuery = query(
          collection(db, "clubs", currentClubId, "players")
        );
  ...
  const handleConnectStripe = async () => {
    if (!clubId) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "No se pudo identificar tu club.",
      });
      return;
    }
    setConnecting(true);
    ...
    if (result.success && result.url) {
      ...
      toast({
        ...
        title: "Error",
        description:
          result.error || "No se pudo generar el enlace de conexión.",
      });
      setConnecting(false);
    }
  };
  ...
  const handleSaveFeeSettings = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Los ajustes de cobro de cuotas han sido actualizados. Por favor, actualiza la página para ver los cambios.",
      });
    } catch (e) {
      toast({
        variant: "destructive",
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Conecta tu cuenta de Stripe para empezar a cobrar cuotas a tus
          miembros de forma automática.
        </p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Conecta tu cuenta de Stripe para gestionar los pagos de cuotas de
              tus miembros de forma segura a través de nuestra plataforma.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <p className="text-muted-foreground">
                Para empezar, necesitas conectar tu cuenta de Stripe. Serás
                redirigido a Stripe para completar un proceso de onboarding
                seguro.
              </p>
              {!onboardingComplete && (
              ...
              <div className="flex items-center gap-4 p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-sm text-green-600">
                    ¡Todo listo para empezar a gestionar los cobros!
                  </p>
                </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700">
                    Completa el proceso de conexión con Stripe para activar el
                    cobro de cuotas.
                  </p>
                </div>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Define cuándo se realizarán los cobros automáticos de las cuotas a
            los miembros.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select
                value={feeChargeDay.toString()}
                onValueChange={(value) => setFeeChargeDay(Number(value))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {feeChargeMonths.length > 0
                      ? `${feeChargeMonths.length} meses seleccionados`
                      : "Seleccionar meses..."}
                  </Button>
                </PopoverTrigger>
                        ...
                        {MONTHS.map((month) => (
                          ...
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                feeChargeMonths.includes(month.value)
                                  ? "opacity-100"
                                  : "opacity-0"
                              )}
                            />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Consulta y gestiona las cuotas asignadas a cada jugador.
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  filteredPlayers.map((player) => {
                    const monthlyFee =
                      feeChargeMonths.length > 0 && player.annualFee
                        ? (
                            player.annualFee / feeChargeMonths.length
                    ...
                    return (
                      <TableRow key={player.id}>
                        ...
                        <TableCell className="font-medium">
                          {player.annualFee !== null &&
                          player.annualFee !== undefined
                            ? `${player.annualFee.toLocaleString("es-ES", {
                                style: "currency",
                                currency: "EUR",
                              })}`
                            : "No asignada"}
